...
#[allow(dead_code)]
impl Vector {
    pub(crate) fn new(start: Point<i32>, end: Point<i32>) -> Vector {
        Vector { start, end }
    }
    ...
    pub(crate) fn dot(&self, other: &Vector) -> i32 {
    ...
    pub(crate) fn compare_direction(&self, other: &Vector) -> bool {
        let x1 = self.end.x - self.start.x;
        let y1 = self.end.y - self.start.y;
        let x2 = other.end.x - other.start.x;
        let y2 = other.end.y - other.start.y;
    ...
    pub(crate) fn end_points_meet(&self, other: &Vector) -> i8 {
        ((self.start == other.end) as i8) + -((self.end == other.start) as i8)
    }

    pub(crate) fn angle(&self, other: &Vector) -> f64 {
    ...
    pub(crate) fn translate(self, x: i32, y: i32) -> Vector {
        Vector {
            start: Point {
                x: self.start.x + x,
                y: self.start.y + y,
